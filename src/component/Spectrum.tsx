...
export default function Spectrum({
  ...
  const draw = useCallback(() => {
    ...
    if (canvas && analyserNode) {
      ...
      var sliceWidth = canvas.width / processed.length;
      var x = 0;

      for (let i = 0; i < processed.length; i++) {
        // normalize the data to 0 to 1
        let normalized = (processed[i] - 128.0) / 128;
        // thersold to amplify the spectrum
        let thersold = 2;
        // height of the bar , when smaller then 0.01 stay 2 to prevent shaking of bar
        let rectHeight =
          normalized > 0.01 ? normalized * thersold * canvas.height : 2;
